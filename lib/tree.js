...
var _insert = function (values) {
    ...
    if (index === 0) {
      this.root = (data instanceof TreeNode) ? data : new TreeNode(data);
      this.root.__isFulfilled = true;
			...
			while (current) {
				if (current.value > data.value) {
					if (!current.left) {
            current.left = (data instanceof TreeNode) ? data : new TreeNode(data);
            current.left.__isFulfilled = true;
					...
					if (!current.right) {
            current.right = (data instanceof TreeNode) ? data : new TreeNode(data);
...
const BINARY_TREE = class BinaryTree {
    ...
    this.root = (options.root) ? new TreeNode(options.root) : null;
...
/*
//TODO
- Configurable collision behavior ie. Tree, Queue, Hash
- Configurable tree type ie. BinarySearch, Red/Black, Binary
- Error handling
- .remove method given a "value" argument removes node from tree and re-order
- .length property should return the numbers of nodes in tree
